...
void random::install_reader(reader *r) {
    #ifdef _WIN32
...
void random::install_thread(reader *r) {
    install_reader(r);
}

void random::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void random::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void random::__unlock() { ReleaseMutex(mutex); }
#else
    void random::__lock() { pthread_mutex_lock(&mutex); }
    void random::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const random::t &t);
template <>
random::t _arg<random::t>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<random::t>(ivy_ser &res, const random::t&);
template <>
void  __deser<random::t>(ivy_deser &inp, random::t &res);
int random::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
void random::__init(){
}
random::t random::ext__show(t z){
    random::t y;
    y = (t)___ivy_choose(0,"fml:y",0);
    {
        y = x;
        x = z;
    }
...
void random::__tick(int __timeout){
}
random::random(){
#ifdef _WIN32
...
template <>
void  __ser<random::t>(ivy_ser &res, const random::t&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class random_repl : public random {
...
random::t _arg<random::t>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "a") return random::a;
    if(arg.atom == "b") return random::b;
    if(arg.atom == "c") return random::c;
...
public:
    ...
    cmd_reader(random_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "show") {
                    ...
                    __ivy_out  << "= " << ivy.ext__show(_arg<random::t>(args,0,3)) << std::endl;
                }
                else
