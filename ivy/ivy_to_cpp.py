...
#                            raise iu.IvyError(None,'cannot create test generator because type {} is uninterpreted'.format(sym.sort.rng))
    ...
    """.replace('thing',thing).replace('actname',username).replace('methodname',varname(actname)).replace('numargs',str(len(action.formal_params))).replace('getargs',getargs))
                ...
                try:
                    newTestIters = str(raw_input("Input, as an integer, the number of steps for your simulation to take (default is 100): "))
                    # newTestIters = iu.Parameter("test_iters",str(newTestItersUser.strip()))
                except:
                    newTestIters = "100"
                    print "Parameter Error. Defaulting to 100 steps."
                try:
                    newTestRuns = str(raw_input("Input, as an integer, the number of full simulation runs (default is 1): "))
                    # newTestRuns = iu.Parameter("test_iters",str(newTestRunsUser.strip()))
                except:
                    newTestRuns = "1"
                    print "Parameter Error. Defaulting to 1 run."
                ...
                impl.append("        int test_iters = TEST_ITERS;\n".replace('TEST_ITERS',newTestIters))
                impl.append("        int runs = TEST_RUNS;\n".replace('TEST_RUNS',newTestRuns))
